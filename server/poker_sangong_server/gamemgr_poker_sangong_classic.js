...
exports.set_ready = function (user_id, status) {
    ...
    if (game == null) {
        ...
        //确定开几张牌
        var open_card = 0;
        if (global.has_rule(game.conf.rule_index, global.OPNE_CARD_NUM_TWO)) {
            open_card = 2;
        }
...
function game_end(room_id, force, over) {
    ...
    if (!force) {
        ...
        //logger.debug("card type --------------->",banker_card_type);
            ...
            if (i != banker_index) {
                ...
                if (user_seat_data && user_seat_data.user_id > 0) {
                    ...
                    if (pokerUtils.compare(banker_card_type, user_card_type)) {
                        //banker win
                        //console.log("banker win---->",global.SANGONG_BASE_SCORE,(pokerUtils.get_times(banker_card_type)),user_seat_data.bet_coin)
                        var ss = global.SANGONG_BASE_SCORE * (pokerUtils.get_times(banker_card_type)) * user_seat_data.bet_coin;
                        ...
                        if (!user_seat_data.statistic.sangong_type_count[user_card_type.type]) {
                            user_seat_data.statistic.sangong_type_count[user_card_type.type] = 1;
                        } else {
                            user_seat_data.statistic.sangong_type_count[user_card_type.type] += 1;
                        }
                    } else {
                        //user win
                        //console.log("banker win---->",global.SANGONG_BASE_SCORE,(pokerUtils.get_times(banker_card_type)),user_seat_data.bet_coin)
                        var ss = global.SANGONG_BASE_SCORE * (pokerUtils.get_times(user_card_type)) * user_seat_data.bet_coin;
                        if (!score[banker_index]) {
                            score[banker_index] = -ss;
                        } else {
                            score[banker_index] -= ss;
                ...
                if (!banker_seat_data.statistic.sangong_type_count[banker_card_type.type]) {
                    banker_seat_data.statistic.sangong_type_count[banker_card_type.type] = 1;
                } else {
                    banker_seat_data.statistic.sangong_type_count[banker_card_type.type] += 1;
                }
            }
    ...
    //霸王庄，牌大做庄，牛牛做庄这里判断庄家
    if (global.has_rule(game.conf.rule_index, global.SANGONG_BANKER)) {
        //牛牛做庄
        var sangong_index = [];
        for (var a in card_type_list) {
            var card_type = card_type_list[a];
            if (card_type.type == global.SANGONG_HUN_SAN) {
                sangong_index.push(a);
            }
        }
        //
        if (sangong_index.length != 0) {
            //如果有多个牛牛，随机一个庄
            game.button = sangong_index[Math.floor(Math.random() * sangong_index.length)];
            //此处需要更新到数据库
        ...
        for (var i = 0; i < room.seats_num; ++i) {
            ...
            if (seat_data && seat_data.user_id > 0) {
                ...
                var data = {
                    ...
                    sangong_type_count: seat_statistic.sangong_type_count,
                    total_score: seat_statistic.total_score,
                }
...
exports.off_banker = function (user_id, data) {
    ...
    // var can_off = true;
    // if(seat_data.statistic.total_score == (0-global.SANGONG_BANKER_SCORE)){
