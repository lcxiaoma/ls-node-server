...
function get_max_games(conf,double){
	var max_games =0;
	if(global.has_rule(conf.type_index,global.SANGONG_GAME_TYPE_CLASSIC)){
...
function get_poker_nums(conf){
	...
	var nums =52;
	//无花牌是40张
	if(global.has_rule(conf.type_index,global.SANGONG_GAME_TYPE_NONE_COLOR)){
		   nums =52;
	   }
	return nums;
}
...
function constructRoomFromDb(dbdata){
	...
	//for test xzdd
	if(global.has_rule(roomInfo.conf.type_index,global.SANGONG_GAME_TYPE_CLASSIC)){
		roomInfo.gameMgr = require("./gamemgr_poker_sangong_classic");
	}
	else{
