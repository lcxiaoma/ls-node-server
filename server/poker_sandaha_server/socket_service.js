...
exports.start = function(config,mgr){
	...
	io.sockets.on('connection',function(socket){
		...
		//叫庄
		socket.on('sdh_call_banker',function(data){
			show_request('sdh_call_banker',data);
			data = JSON.parse(data);
			var user_id = socket.userId;
			if(user_id == null) return;
			socket.gameMgr.call_banker(user_id,data.call);
		});
		//确定花色
		socket.on('sdh_decide_main_color',function(data){
			show_request('sdh_decide_main_color',data);
			data = JSON.parse(data);
			var user_id = socket.userId;
			if(user_id == null) return;
			socket.gameMgr.decide_main_color(user_id,data.color);
		});
		//埋牌
		socket.on('sdh_bury_card',function(data){
			show_request('sdh_bury_card',data);
			data = JSON.parse(data);
			var user_id = socket.userId;
			if(user_id == null) return;
			socket.gameMgr.bury_card(user_id,data.cards);
		});
		//认输
		socket.on('sdh_give_up',function(data){
			show_request('sdh_give_up',data);
			data = JSON.parse(data);
			var user_id = socket.userId;
			if(user_id == null) return;
			socket.gameMgr.give_up(user_id);
		});
		//出牌
		socket.on('out_card',function(data){
			show_request('out_card',data);
			if(socket.userId == null){
				return;
			}
			socket.gameMgr.out_card(socket.userId,JSON.parse(data));
		});
	...
	logger.info("POKER SANDAHA SOCKET RUNNING ON:" + config.CLIENT_PORT);	
};