...
function get_seats_num(conf){
	var seats_num =4;
	if(global.has_rule(conf.rule_index,global.PLAYER_3)){
		seats_num =3;
	}
	if(global.has_rule(conf.rule_index,global.PLAYER_4)){
		seats_num =4;
	}
...
function get_poker_nums(conf){
	...
	var nums =52;
	//无花牌是40张
	if(global.has_rule(conf.type_index,global.SHISSHUI_GAME_TYPE_CLASSIC)){
		   nums =52;
	   }
	return nums;
}
...
function constructRoomFromDb(dbdata){
	...
	//for test xzdd
	if(global.has_rule(roomInfo.conf.type_index,global.SHISSHUI_GAME_TYPE_CLASSIC)){
		roomInfo.gameMgr = require("./gamemgr_poker_shisanshui_classic");
	}
	else{
