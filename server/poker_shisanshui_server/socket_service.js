...
function gm_command(socket,comond){
	console.warn("GM commond ======>",comond);
	if(comond[0] == "chose1"){
		var test_data = require("./test").check_data
		socket.gameMgr.chose(socket.userId,JSON.stringify(test_data.type_none.data1));
	}
	if(comond[0] == "chose2"){
		var test_data = require("./test").check_data
		socket.gameMgr.chose(socket.userId,JSON.stringify(test_data.type_none.data2));
	}
}
...
exports.start = function(config,mgr){
	...
	io.sockets.on('connection',function(socket){
		...
		// //下庄
		// socket.on('off_banker',function(data){
		// 	show_request('call_banker',data);
		// 	var user_id = socket.userId;
		// 	data = JSON.parse(data);
		// 	if(user_id== null) return;
		// 	socket.gameMgr.off_banker(user_id,data);
		// });
		...
		socket.on('chose',function(data){
			show_request('chose',data);
			var user_id = socket.userId;
			if(user_id == null) return;
			socket.gameMgr.chose(user_id,data);
		});
		//聊天
	...
	logger.info("POKER OX SOCKET RUNNING ON: " + config.CLIENT_PORT);	
};