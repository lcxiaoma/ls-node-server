...
function dispatch_card(game){
    ...
    //         if(i== game.button){
    //             seat_data.holds =[1,7,9,22,23]
    //         }else{
    //             seat_data.holds =[1,1,1,1,2]
    //         }
    //     }
    // }
    // return;
...
exports.set_ready = function(user_id,status){
    ...
    if(game ==null){
       ...
       // logger.debug("has game check can begin  less_begin =%s",game.less_begin);
        ...
        if(game.state == global.GAME_STATE_BANKER_BASE){
            var bbs_msg = JSON.parse(msg_templete.SC_BankerBaseStatus);
            bbs_msg.chose_index =game.button;
            bbs_msg.base_score = global.OX_OX_BANKER_BASE;
            userManager.sendMsg(user_id,'banker_base_status',bbs_msg);
        }
        if(game.base_score !=0 ){
            notice_banker_status(game,user_id);
        }
...
function init_game_base(room_id){
    ...
    if(game == null){
        ...
        game = {
            ...
            current_banker_count:roominfo.change_info.current_banker_count,
            current_banker_score:roominfo.change_info.current_banker_score,
            banker_circle:roominfo.change_info.banker_circle,
            base_score:roominfo.change_info.base_score,
        }
...
function begin (room_id){
    ...
    if(game == null){
        ...
        game = {
            ...
            turn:0,
            state:global.GAME_STATE_FREE,
            //less_begin:roominfo.less_begin,//是否可以提前开始
            less_begin:roominfo.change_info.less_begin,
			...
			// 				banker_circle:[0,0],
			// 			}
            current_banker_count:roominfo.change_info.current_banker_count,//当前庄的轮数
            current_banker_score:roominfo.change_info.current_banker_score,//当前庄家基础分
            banker_circle:roominfo.change_info.banker_circle,//游戏圈数
            base_score:roominfo.change_info.base_score,//公盘基础分
        }
        // //for test
    ...
    if(game.game_index == 1){
        game.button =0;
        game.state = global.GAME_STATE_BANKER_BASE;
    }else{
        if(game.base_score ==0 && game.current_banker_count ==0){
            game.state = global.GAME_STATE_BANKER_BASE;
        }else{
    ...
    for(var i=0;i<player_num;++i){
        if((game.banker_circle[0] & (0x01 <<(i)))&& circle ==0){
            circle =1;
        }
        if(game.banker_circle[1] & (0x01 <<(i))){
            circle =2;
            break;
        }
    }
    ...
    for(var i=0;i<seats.length;++i){
        ...
        //TIPS 斗公牛，这里可能要做成圈数
        // userManager.sendMsg(s.user_id,'game_num_push',circle);        
    ...
    if(game.state == global.GAME_STATE_BANKER_BASE){
        //TIPS斗公牛特殊标记用于记录，玩家这圈是否当过庄(仅限于第一次庄)
        var circle1 =game.banker_circle[0];
        circle1 += 0x01 << (game.button);
        game.banker_circle[0] = circle1;
        game.room_info.change_info.banker_circle[0] = circle1;

        var bbs_msg = JSON.parse(msg_templete.SC_BankerBaseStatus);
        bbs_msg.chose_index =game.button;
        bbs_msg.base_score = global.OX_OX_BANKER_BASE;
        userManager.send_to_room('banker_base_status',bbs_msg,game.room_info.id);
    }
...
function game_end (room_id,force,over){
    ...
    if(!force){
        ...
        //当庄家分数为负数的时候处理
        if(game.current_banker_score +score[banker_index] <0){
            //庄家分数为负数的时候 得重新分配分数
            var all_score = game.current_banker_score + banker_win;
            //房子除0
            banker_lose = (banker_lose ==0)?1:banker_lose;

            //20170515如果庄家分不够分配，按照从大到小依次分配
            var tmp_max =[];
            for(var b in card_type_list){
                if(b != game.button ){
                    if(score[b] >0){
                        tmp_max.push(card_type_list[b]);
                    }else{
                        //输的玩家将全额支付
                        var seat_info = game.game_seats[b];
                        if(seat_info && seat_info.user_id >0){
                            seat_info.statistic.total_score += score[b];
                        }
                    }
                    
                }else{
                    //设置庄家分
                    score[game.button] = -all_score;
                    game.current_banker_score =0;
                    game.room_info.change_info.current_banker_score =0;
                }
            }

            tmp_max.sort(function(a,b){return pokerUtils.compare(b,a)});
            var tmp_max_index =[];
            for(var i=0;i<tmp_max.length;++i){
                for(var a in card_type_list){
                    if(card_type_list[a] == tmp_max[i]){
                        tmp_max_index.push(a);
                    }
                }
            }
            // console.log("show me score =============>",score);
            // console.log("show me all score=========>",all_score)
            // console.log("show me temp max index======>",tmp_max_index)
            for(var j=0;j<tmp_max_index.length;++j){
                var index = tmp_max_index[j];
                var seat_info = game.game_seats[index];
                if(seat_info && seat_info.user_id >0){
                    // console.log("show me info=======>",all_score -score[index]);
                    // console.log("show =================>",score[index])
                    if(all_score -score[index] <0){
                        if(all_score >0){
                            score[index] = all_score;
                            seat_info.statistic.total_score += all_score;
                            all_score  =0;
                        }else{
                            score[index] =0;
                        }
                    }else{
                        all_score -=score[index];
                        seat_info.statistic.total_score += score[index];
                    }
                }
            }
            //console.log("show me score after====>",score);
            // for(var a in score){
            //     if(a != game.button){
            //         var seat_info = game.game_seats[a];
            //         if(seat_info){
            //             if(score[a] >0){
            //                 //玩家赢
            //                 //如果分数不足，重新分配
            //                 var ss = Math.round(score[a] /banker_lose *all_score);
            //                 seat_info.statistic.total_score += ss;
            //                 score[a] = ss;

            //             }else{
            //                 //玩家输，全额付出
            //                 seat_info.statistic.total_score +=score[a];
            //             }
            //         }
            //     }else{
            //         score[a] = -all_score;
            ...
            for(var a in score){
                if(a != game.button){
                    ...
                    game.current_banker_score += score[a];
                    game.room_info.change_info.current_banker_score += score[a];
                }
            }
        }
    }else{
        //强制解散也要加上庄家的结算
        game.game_seats[game.button].statistic.total_score += game.current_banker_score;
    }
    //end_seats_data
    //user_id
    //holds
    ...
    var circle_end = true; //游戏圈数是否结束
    //斗公牛公盘为0的时候自动下庄
    if(game.current_banker_score ==0){
        //判断圈数决定是否游戏已经结束
        var player_num =get_game_realplayernum(game);
        ...
        if(game.conf.max_games ==1){
            for(var i=0;i<player_num;i++){
                // console.log("??????????????????????",game.banker_circle[0],game.room_info.change_info.banker_circle[0])
                // console.log("player num %d  , game.button === %d",player_num,game.button);
                //位运算重复叠加会出错
                if(game.banker_circle[0] <=sum_mask){
                    if(!(game.banker_circle[0] & (0x01 <<(i)))){
                        circle_end = false;
                        break;
                    }
                }
        ...
        }else if(game.conf.max_games ==2){
            if(game.banker_circle[a] <=sum_mask){
                for(var i=0;i<player_num;++i){
                    if(!(game.banker_circle[1] & (0x01 <<(i)))){
                        circle_end = false;
                        break;
        ...
        if(!circle_end){
            //自动下庄
            var tmp_button = (game.button+1)%player_num;
            // console.log("><<<><><><><><><><><>>",game.button,player_num,tmp_button)            
            //判断是否进入下一圈,还是在当前圈
            ...
            //console.log("current circle =%d ,circle1 = %d,circle2= %d",current_circle,circle1_count,circle2_count)
            if(circle1_count == player_num){
                if(current_circle == 0){
                    current_circle =1;
                }
            }
            ...
            //没切换一次庄家，公共积分都要重新选择
            game.button = tmp_button;
            game.current_banker_count =0;
            game.current_banker_score =0;
            game.room_info.change_info.current_banker_score =0;
            game.base_score =0;
            //game.game_seats[game.button].statistic.total_score -= game.base_score;
            //game.current_banker_score = game.base_score;
            //game.banker_circle[current_circle] += (0x01 <<(game.button));
            game.room_info.change_info.current_banker_index = game.button;
            game.room_info.change_info.current_banker_count =0;
            //game.room_info.change_info.current_banker_score = game.base_score,
            game.room_info.change_info.banker_circle[current_circle] += (0x01 << (game.button)); 
            //console.log("show me added info =================>",game.banker_circle)
    ...
    //poker数组
    //是否写入历史记录
    //TIPS 只需要判断是否达到N圈庄，游戏结束
    if(circle_end || (force && over)){
        is_game_over = true
    }
    ...
    if(!is_game_over){
        ...
        if(global.has_rule(game.conf.rule_index,global.TURN_BANKER) || global.has_rule(game.conf.rule_index,global.GRAB_BANKER)){
            if(seat_data && seat_data.user_id >0 && game.current_banker_count >=3 && game.current_banker_score >=game.base_score *3){
...
function init_game_base_info(game){
    ...
    data.change_info ={
        ...
        current_banker_score:game.current_banker_score,
        base_score:game.base_score,
        banker_circle:game.banker_circle,
    }
    for(var i=0;i<game.game_seats.length;++i){
...
exports.init_game_from_db = function(db_data,room_info){
    ...
    if(game == null){
        ...
        game ={
            ...
            current_banker_score:change_info.current_banker_score,
            base_score:change_info.base_score,
            banker_circle:change_info.banker_circle,
        }
        //初始化其它信息
        for(var i=0;i<room_info.seats_num;++i){
            var seat = seats[i];
...
exports.betting = function(user_id,data){
    ...
    var bet_coin = [];
    if(game.base_score ==20){
        bet_coin = [2,4,6,8]
    }else if(game.base_score ==40){
        bet_coin =[4,8,12,16]
    }else if(game.base_score == 60){
        bet_coin = [6,12,18,24];
    }else{
        logger.warn('betting game base score not in config [%d]',game.base_score);
        return;
    }
...
//广播庄家公盘信息
function notice_banker_status(game,user_id){
    if(!user_id){user_id = null;};
    var msg = JSON.parse(msg_templete.SC_BankerStatus);
    msg.base_score = game.base_score;
    msg.banker_count=game.current_banker_count;
    msg.current_score=game.current_banker_score;
    msg.banker_total_score = game.game_seats[game.button].statistic.total_score;
    if(user_id){
        userManager.sendMsg(user_id,'banker_status',msg);
        return;
    }
    userManager.send_to_room('banker_status',msg,game.room_info.id);
}

//第一个庄家选择公盘
exports.banker_base = function(user_id,data){
    var game = get_game_by_user(user_id);
    if(game == null) return;
    

    if(game.state != global.GAME_STATE_BANKER_BASE){
        logger.warn('not in banker base status.')
        return;
    }
    ...
    if(seat_index != game.button){
        logger.warn("not banker can not chose base score.",seat_index,user_id);
        return;
    }

    var base_score = Number(data.base_score);
    //console.log(base_score)
    if(global.OX_OX_BANKER_BASE.indexOf(base_score) == -1){
        logger.warn("not in base score list.");
        return;
    }
    ...
    //设置游戏属性
    game.base_score = base_score;
    game.current_banker_score = base_score;
    seat_data.statistic.total_score -= base_score;
    //TODO设置房间可变属性
    game.room_info.change_info.current_banker_score = base_score;
    game.room_info.change_info.base_score = base_score;

    update_game_info(game);
    update_room_change_info(game);

    var msg_banker_base = JSON.parse(msg_templete.SC_BankerBase);
    msg_banker_base.banker_index = game.button;
    msg_banker_base.base_score = base_score;
    
    userManager.send_to_room('banker_base',msg_banker_base,game.room_info.id);
...
exports.off_banker = function(user_id,data){
    ...
    // }
    // if(game.current_banker_score ==0){
    //     //共盘分数输完,自动下庄.(在game end中做)
    // }
    if((game.current_banker_count >=3 && game.current_banker_score >= game.base_score *3)){
        //或者3局后公盘分数达到基础倍数的3倍的时候可以下庄
        //通知客户端下庄
        ...
        //console.log("show me i need info ===========:>",game.banker_circle,player_num,game.conf.max_games)
        //游戏结束
        if(is_game_over){
            game_over(game.room_info.id,false,true);
            return;
        }
        ...
        //console.log("current cicle = %d ,circle 1 = %d,cicle2 = %d",current_circle,circle1_count,circle2_count)
        game.game_seats[game.button].statistic.total_score += game.current_banker_score;
        ...
        //扣除庄家的基础分
        game.game_seats[game.button].statistic.total_score -= game.base_score;
        //console.log("show me added ===============>",game.banker_circle,game.button)
        //update_game_info(game);
        update_room_change_info(game);
        notice_banker_status(game);   
    }
