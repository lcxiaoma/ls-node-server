...
function get_max_games(conf,double){
	...
	if(global.has_rule(conf.type_index,global.OX_GAME_TYPE_OX) || global.has_rule(conf.type_index,global.OX_GAME_TYPE_OX_NONE_COLOR)){
		if(global.has_rule(conf.rule_index,global.ONE_CIRCLE_BANKER)){
			max_games =1;
		}
		if(global.has_rule(conf.rule_index,global.TWO_CIRCLE_BANKER)){
			max_games =2;
		}
	}else{
...
function get_poker_nums(conf){
	...
	var nums =52;
	//无花牌是40张
	if(global.has_rule(conf.type_index,global.OX_GAME_TYPE_NONE_COLOR) ||
	   global.has_rule(conf.type_index,global.OX_GAME_TYPE_OX_NONE_COLOR)){
		   nums =40;
	   }
	return nums;
}
...
function constructRoomFromDb(dbdata){
	...
	//for test xzdd
	if(global.has_rule(roomInfo.conf.type_index,global.OX_GAME_TYPE_CLASSIC)){
		roomInfo.gameMgr = require("./gamemgr_poker_ox_classic");
	}
	else if(global.has_rule(roomInfo.conf.type_index,global.OX_GAME_TYPE_NONE_COLOR)){
		roomInfo.gameMgr = require("./gamemgr_poker_ox_none_color")
	}
	else if(global.has_rule(roomInfo.conf.type_index,global.OX_GAME_TYPE_OX)){
		roomInfo.gameMgr = require("./gamemgr_poker_ox_ox");
	}
	else if(global.has_rule(roomInfo.conf.type_index,global.OX_GAME_TYPE_OX_NONE_COLOR)){
		roomInfo.gameMgr = require("./gamemgr_poker_ox_ox_none_color");
	}
	else{
