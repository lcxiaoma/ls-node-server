...
exports.start = function(config,mgr){
	...
	io.sockets.on('connection',function(socket){
		...
		socket.on('open_card',function(data){
			show_request('open_card',data)
			var user_id = socket.userId;
			if(user_id == null) return;
			socket.gameMgr.open_card(user_id,data);
		});

		//查牌
		socket.on('compare',function(data){
			show_request('compare',data);
			var user_id = socket.userId;
			if(user_id == null) return;
			data = JSON.parse(data);
			socket.gameMgr.compare(user_id,data)
		});

		//弃牌
		socket.on('giveup',function(data){
			show_request('giveup',data);
			var user_id = socket.userId;
			if(user_id == null) return;
			socket.gameMgr.give_up(user_id,data);
		});
	...
	logger.info("POKER GOLD FLOWER SOCKET RUNNING ON:" + config.CLIENT_PORT);	
};