...
function get_card_nums(conf){
	var nums =3;//金花固定3张牌，无大小王
	return nums;
}

function get_base_score(conf){
	var base_score =2;
	if(global.has_rule(conf.rule_index,global.BASE_SCORE_5)){
		base_score =5;
	}
	if(global.has_rule(conf.rule_index,global.BASE_SCORE_10)){
		base_score =10;
	}
	return base_score;
}
...
function constructRoomFromDb(dbdata){
	...
	//for test xzdd
	if(global.has_rule(roomInfo.conf.type_index,global.GOLDFLOWER_GAME_TYPE_CLASSIC)){
		roomInfo.gameMgr = require("./gamemgr_poker_goldflower_classic");
	}
	else{
				...
				if(ret){
					...
					var roomInfo = {
						...
						conf:{
							...
							base_score:get_base_score(roomConf),
						},
