...
function get_base_score(conf){
	var score =1000;
	if(global.has_rule(conf.rule_index,global.BASE_SCORE_2)){
		score =2000;
	}
	if(global.has_rule(conf.rule_index,global.BASE_SCORE_3)){
		score =3000;
	}
	return score;
}
...
function constructRoomFromDb(dbdata){
	...
	var tmp_change_info = {};
	if(tmp_change_info && ""!= dbdata.room_change_info){
	...
	var roomInfo = {
		...
		watch_seats:new Array(get_seats_num(config_data)),
		change_info:tmp_change_info,
	};
	//设置是否允许提前开始游戏
	//roomInfo.conf.less_begin = dbdata.next_button;
	//for test xzdd
	if(global.has_rule(roomInfo.conf.type_index,global.TAXAS_GAME_TYPE_CLASSIC)){
		roomInfo.gameMgr = require("./gamemgr_poker_taxas_classic");
	}
	else{
...
exports.enterRoom = function(roomId,user_info,callback){
	...
	var fnTakeSeat = function(room){
		...
		if(real_seat_num >= room.seats_num){
			...
			var base_score = get_base_score(room.conf);
			var new_seat_data = {
				user_id:userId,
				name:userName,
				score:base_score,
			...
			for(var j=0;j<room.seats_num;++j){
				...
				if(room.watch_seats[j] && room.watch_seats[j].user_id>0){
					var d ={
						...
						empty:room.watch_seats[j].empty,
