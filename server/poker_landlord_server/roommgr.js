...
function get_poker_nums(conf){
	...
	if(global.has_rule(conf.rule_index,global.MASK_CN_P2)){
		nums = 54;  //108  //两副牌功能关闭，强制改为一副牌  at 2017-07-10
	}
	return nums;
}
...
function get_card_nums(conf){
	...
	if(global.has_rule(conf.rule_index,global.MASK_CN_P2)){
		nums =(54-3)/3;    //(108-6)/3  //两副牌功能关闭，强制改为一副牌  at 2017-07-10
	}
	return nums;
}
...
function constructRoomFromDb(dbdata){
	...
	//for test xzdd
	if(global.has_rule(roomInfo.conf.type_index,global.LANDLORD_GAME_TYPE_CLASSIC)){
		roomInfo.gameMgr = require("./gamemgr_poker_landlord_classic");
	}
	else if (global.has_rule(roomInfo.conf.type_index,global.LANDLORD_GAME_TYPE_CHANGE)){
		roomInfo.gameMgr = require("./gamemgr_poker_landlord_change");
	}
	else{
		...
		if(real_seat_num >= room.seats_num){
			...
			for(var j=0;j<seat_info_length;++j){
				...
				var d ={
					...
					statistic:room.seats[i].statistic,
				}
				seat_info.push(d);
			}
			var data ={
