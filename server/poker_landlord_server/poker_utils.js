...
//找出num个相同的
function find_num_same(card_value,num){
...
//检测牌类型是否正确
//都没确认王牌是能当成对子打或者炸弹打
exports.check_card_type = function(base_card_type,base_card,card_type,out_card,is_last,double_king){
    if(is_last != true){is_last = false;};
    if(double_king != true){double_king = false;};
    var action = false;
    ...
    if(base_card_type == card_type || base_card_type ==0){
        ...
        switch (card_type) {
            ...
            case global.MORE_DOUBLE:
                action = (out_value.length >=2*3 && out_value.length %2==0)?true:false;
                if(!action) return action;
            ...
            case global.SEQUNECE:
                ...
                break;
            case global.THREE_PICK_TWO://3代一对
                // if(is_last){
                ...
                //     if(!action) return action;
                // }
                if(out_value.length !=5) return false;
                ...
                var double_list = []
                for(var a in out_value){
                    var value = out_value[a];
                    if(value != out_t[0]){
                        double_list.push(value)
                    }
                }
                var is_double = true;
                for(var b in double_list){
                    if(double_list[0] !=double_list[b]){
                        is_double = false;
                    }
                }
                ...
                break;
            case global.MORE_THREE_PICK_TWO://飞机能带2对，或者带2个
                //
                if(out_value.length %5!=0 && out_value.length %4 !=0 &&out_value.length <=4){
                    return false;
                }
                var is_pick_double = false;
                if(out_value.length %5 ==0){
                    is_pick_double = true;
                }
                ...
                if(base_card_type !=0){
                    ...
                    var fly_length = (is_pick_double)?base_value.length /5:base_value.length/4;
                    ...
                    if(is_pick_double){
                        ...
                        var double_list=[]
                        for(var a in out_value){
                            if(out_three_list.indexOf(out_value[a]) == -1){
                                double_list.push(out_value[a])
                            }
                        }
                        var double_value ={};
                        for(var b =0;b<double_list.length;++b){
                           if(!double_value[double_list[b]]){
                               double_value[double_list[b]] =1;
                           }else{
                                double_value[double_list[b]] +=1;
                           }
                        }

                        // console.log("double list ==========>",double_list);
                        // console.log("double value =========>",double_value);
                        for(var c in double_value){
                            if(double_value[c]%2 !=0){
                                return false;
                            }
                        }
                    ...
                    //首出牌，只需要检测出牌的内容是否合法
                    var fly_length = (is_pick_double)?out_value.length /5:out_value.length/4;
                    console.log("show me fly length ===========>",fly_length)
                    var three_list =check_more_three_value(out_value,fly_length);
                    if(is_pick_double){
                        //检测代的是否是对子
                        var double_list=[]
                        for(var a in out_value){
                            if(three_list.indexOf(out_value[a]) == -1){
                                double_list.push(out_value[a])
                            }
                        }
                        console.log("three pick double three value ===",three_list)
                        var double_value ={};
                        for(var b =0;b<double_list.length;++b){
                           if(!double_value[double_list[b]]){
                               double_value[double_list[b]] =0;
                           }
                           double_value[double_list[b]] +=1;
                        }
                        console.log("three pick double double value ===",double_value)
                        for(var c in double_value){
                            if(double_value[c]%2 !=0){
                                return false;
                            }
            ...
            case global.BOOM:
                ...
                if(double_king){
                    if(out_value.length ==2){
                        if(out_value[0] ==global.KING_VALUE && out_value[1] == global.KING_VALUE){
                            return true;
                        }
                        return false;
                    }
            ...
            case global.SAME_SEQUNECE:
                ...
                return (base_value[0] <out_value[0]||base_card_type ==0)?true:false;
                break;
            case global.THREE://3不代
                if(out_value.length!=3) return false;
                for(var i=1;i<out_value.length;++i){
                    if(out_value[0] != out_value[i]){
                        return false;
                    }
                }
                if(base_card_type ==0){
                ...
                if(out_value[0] > base_value[0]) return true;
                return false;
                break;
            case global.THREE_PICK_ONE://3代1
                if(out_value.length !=4) return false;

                var out_three =check_three_value(out_value);

                if(out_three.length ==0) return false;

                if(base_card_type ==0){
                    return true;
                }
                var base_three = check_three_value(base_value);

                if(out_three[0] > base_three[0]) return true;
                return false;
                break;
            case global.FOUR_PICK_TWO://4代2
                if(out_value.length != 6) return false;
                
                var out_four_list =find_num_same(out_value,4);
                //console.log("four pick two four list --->",out_four_list)
                if(out_four_list.length ==0) return false;
                
                if(base_card_type ==0) return true;

                var base_four_list = find_num_same(base_value,4)
                if(out_four_list[0] > base_four_list[0]) return true;
                return false;
                break;
        }
    }else{
        if(card_type == global.PASS){
            ...
            //check BOOM
            if(double_king &&out_value.length ==2 ){
                if(out_value[0]==global.KING_VALUE &&out_value[1]==global.KING_VALUE){
                    return true;
                }
                return false;
