...
exports.betting = function (user_id, data) {
	...
	if (bet_type == 1) {
		...
		// 最高下注1000
		if (seat_data.bet_coin + data.coin > 1000) {
			msg.error_code = error.USER_COIN_NOT_INVALID;
			userManager.sendMsg(user_id, "betting", msg);
			return;
		}
	...
	if (all_bet) {
		...
		for (var i = 0; i < game.game_seats.length; ++i) {
			...
			if (seat_data && seat_data.user_id > 0) {
				...
				var card_type1 = pokerUtils.check_card_type(seat_data.holds.slice());
				seat_data.card_type = { type: card_type1.type, value: card_type1.max_value };
...
function move_to_next_action(game) {
	...
	// if (next_seat.seat_index == game.button && next_seat.card_type.type == global.BLACK_JACK) {
...
function dispatch_1_card(game, target, seat_index) {
	...
	game.current_index += 1;
	// 压入手牌
	game.game_seats[seat_index].holds.push(poker);
	return poker;
}
...
function check_can_action(seat_data) {
	...
	if (global.has_rule(mask, global.USER_STATE_SPLIT)) {
		...
		// 	right += 1;
		// }
		var ct1 = seat_data.card_type;
		if (ct1.type == global.POINT10_TYPE_BOOM || ct1.value >= 10.5 || ct1.type == global.POINT10_TYPE_9S || ct1.type == global.POINT10_TYPE_9S) {
			right += 1;
		}
...
function hit(game, seat_data, player_which_hand, msg, send_func) {
	...
	var old_card_type = seat_data.card_type;
	if (!old_card_type) {
		return;
	}
	// if (old_card_type.type == global.BUST || old_card_type.type == global.EQUAL_21 || old_card_type.type == global.BLACK_JACK || old_card_type == global.FIVE_DRAGON) {
	// 	return;
	// }
	if (old_card_type.type == global.POINT10_TYPE_BOOM || old_card_type.value >= 10.5 || old_card_type.type == global.POINT10_TYPE_9S || old_card_type.type == global.POINT10_TYPE_K9P) {
		return;
	}
	...
	send_func(seat_data.user_id, game.room_info.id, msg);
	notice_user_holds_change(seat_data.user_id, seat_data.holds, []);
	var tmp_card_type = pokerUtils.check_card_type(seat_data.holds);

	// logger.debug('tmp_card_type', tmp_card_type, 'seat_data holds: ', seat_data.holds)
	seat_data.card_type = { type: tmp_card_type.type, value: tmp_card_type.max_value };
...
function double(game, seat_data, player_which_hand, msg, send_func) {
	...
	// 	return;
	// }
	if(global.has_rule(seat_data.status_mask,global.USER_STATE_DOUBLE)){
		return;
	}
	...
	var new_card_type = pokerUtils.check_card_type(seat_data.holds);
	seat_data.card_type = { type: new_card_type.type, value: new_card_type.max_value };
	var old_bet_coin = seat_data.bet_coin;
	seat_data.bet_coin += old_bet_coin;
	
	seat_data.status_mask += 0x01 << global.USER_STATE_DOUBLE;
...
function broadcast_user_state(room_id, seat_data, banker, open) {
	...
	if (banker && !open) {
		...
		msg.type = seat_data.card_type.type;
		msg.value = seat_data.card_type.value;
	}
	msg.status_mask = seat_data.status_mask;
	userManager.send_to_room('point105_state', msg, room_id);
}
...
exports.set_ready = function (user_id, status) {
	...
	if (game == null) {
		...
		for (var i = 0; i < room_info.seats_num; ++i) {
			...
			//庄家特殊处理，手牌，手牌类型。
			var display_holds = []
			// var card_type = pokerUtils.check_card_type(seat_data.holds.slice());
			if (user_id == seat_data.user_id) {
				display_holds = seat_data.holds
			} else {
				display_holds = get_crypt_list(seat_data.holds.length)
			}
			var tmp = {
...
function game_end(room_id, force, over) {
	...
	if (!force) {
		//非强制结算才需要去计算分
		var banker_seat = game.game_seats[game.button];
		var banker_card_type = pokerUtils.check_card_type(banker_seat.holds);
		...
		for (var i = 0; i < game.game_seats.length; ++i) {
			if (i != game.button) {
				...
				if (user_seat && user_seat.user_id > 0) {
					//确认玩家真实存在就比较大小计算分值
					var user_card_type = pokerUtils.check_card_type(user_seat.holds);
					card_type_list[i] = user_card_type;
					//获取到了牌型，比较大小，确定输赢和倍率

					var r = pokerUtils.compare(banker_card_type, user_card_type);
					...
					if (r == true) {
						...
						score[game.button] += user_seat.bet_coin * tt ;
						score[i] -= user_seat.bet_coin * tt ;

					}

					if (r == false) {
						//玩家赢
						var tt = pokerUtils.get_times(user_card_type);
	...
	for (var i = 0; i < len; ++i) {
		...
		if (seat_data && seat_data.user_id > 0) {
			...
			// console.error(card_type_tmp,">>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			if (card_type_tmp) {

				if (!seat_statistic.point105_type_count[card_type_tmp.type]) {
					seat_statistic.point105_type_count[card_type_tmp.type] = 0;
				}
				seat_statistic.point105_type_count[card_type_tmp.type] += 1;

				card_type.type = card_type_tmp.type;
				card_type.value = card_type_tmp.max_value;
			}

			// console.error("seat_statistic >>>>>>>>>>>>>>>>",seat_statistic)
	...
	if (game == null) {
		...
		for (var i = 0; i < room.seats_num; ++i) {
			...
			if (seat_data && seat_data.user_id > 0) {
				...
				// 结算数据, 针对每个玩家
				// console.error(seat_statistic,"game over seat statistic =============>")
				var data = {
					banker_count: seat_statistic.banker_count,
					point105_type_count: seat_statistic.point105_type_count,
					total_score: seat_statistic.total_score,
				}
...
exports.init_game_from_db = function (db_data, room_info) {
	...
	if (game == null) {
		...
		for (var i = 0; i < room_info.seats_num; ++i) {
			...
			data.card_type = {};
			if (holds[i].length == 1) {
				var ct_t1 = pokerUtils.check_card_type(holds[i]);
				data.card_type = { type: ct_t1.type, value: ct_t1.max_value };
			}
			// if (holds[i].length == 2) {
...
function notice_user_holds_change(user_id, holds, folds, show_all) {
	...
	var player_holds = seat_data.holds.slice()
	var crypto_hold = get_crypt_list(player_holds.length)
		...
		if (seat && seat.user_id > 0) {
			if (show_all) {
				...
				if (seat.user_id == user_id) {
					...
					crypt_msg.holds = crypto_hold
				}
			}
			userManager.sendMsg(seat.user_id, 'game_holds_push', crypt_msg);
		}
	}

	for (var i = 0; i < game.room_info.watch_seats.length; ++i) {
		var w_seat = game.room_info.watch_seats[i];
		if (w_seat && w_seat.user_id > 0) {
			if (show_all) {
				crypt_msg.holds = player_holds
			} else {
				crypt_msg.holds = crypto_hold;
			}
			userManager.sendMsg(w_seat.user_id, 'game_holds_push', crypt_msg);
		}
	}
}
